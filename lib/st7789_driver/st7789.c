...
esp_err_t st7789_init(void) {
  ...
  // Inicializamos el SPI indicando el puerto que controla el bus, su config y
  // la dirección de la DMA
...
void st7789_send_buffer(const uint8_t *buffer, size_t size) {
  ...
  // Esto evita que el DMA colapse o que no haya memoria contigua para
  // bounce-buffers con Flash
  size_t chunk_size = 240 * 40 * 2;
  size_t offset = 0;

  while (offset < size) { // Limitamos la memoria
    size_t current_chunk =
        (size - offset > chunk_size) ? chunk_size : (size - offset);
...
void st7789_draw_bitmap(uint16_t x, uint16_t y, uint16_t w, uint16_t h,
  ...
  // Si el bloque cabe perfectamente en la pantalla, enviar todo el bloque de
  // una (Max Performance)
  if (draw_w == w && draw_h == h) {
    size_t send_size = w * h * 2;
    lcd_data(spi, (const uint8_t *)bitmap, send_size);
  }
  // Si se corta en el borde de la pantalla, enviar fila por fila evadiendo el
  // "Ghosting / Wrapping"
  else {
    for (int row = 0; row < draw_h; row++) {
      // Saltamos a la fila correspondiente usando el Ancho original (w) del
      // bitmap en memoria
