...
\begin{frame}{Ecosistema de Hardware: 6 sensores integrados}
    \begin{columns}[T]
        ...
        \includegraphics[width=\linewidth, keepaspectratio]{Entrega1/diag_ecosistema.pdf}
        \vspace{0.05cm}
        \scriptsize\textit{Diagrama del ecosistema (reutilizado del informe 10\,\%).}
    \end{columns}
\end{frame}
...
\begin{frame}{Subsistema de Energ\'ia y PCB Prototipo}
    \begin{columns}[T]
        ...
        \fcolorbox{black!30}{black!05}{\parbox{0.95\linewidth}{\centering
            \vspace{1.5cm}
            \scriptsize\textit{foto de PCB through-hole\\(pendiente)}
            \vspace{1.5cm}
        }}
        \vspace{0.2cm}
        \scriptsize \textit{PCB through-hole con C3 SuperMini en montaje; en paralelo se encarg\'o la revisi\'on siguiente con ESP32-S3 (slide 12).}
    \end{columns}
\end{frame}
...
\begin{frame}{Stack BLE y herramienta de validaci\'on}
    \begin{columns}[T]
        ...
        \vspace{0.1cm}
        \scriptsize
        \textbf{supaclock\_monitor.py} muestra biometr\'ia en vivo y graba CSV.\\
        \textbf{$\rightarrow$ Base de datos para entrenar clasificador CNN 1D.}
    \end{columns}
\end{frame}

% ─── Slide 11 — Optimización energética: antes / después ───
\begin{frame}{Optimizaci\'on energ\'etica: antes vs despu\'es}
    \begin{columns}[T]
        ...
        \scriptsize \textit{TX3 marca $\sim$20\,mA AC con pant. off: esto por bursts del light-sleep.}
        ...
        \vspace{0.1cm}
        \scriptsize\textit{Pico $\sim$5$\times$ menor ($110\to 20$\,mA reposo). CPU ya no satura: el bottleneck para ML pasa a ser \textbf{memoria}.}
    \end{columns}
\end{frame}

% ─── Slide 12 — Migración a ESP32-S3 ───
\begin{frame}{Migraci\'on al Seeed XIAO ESP32-S3 (encargada)}
    ...
    \begin{columns}[T]
        ...
        \scriptsize \textit{Escala lineal: la PSRAM externa del S3 abre $\sim$25$\times$ m\'as memoria que el C3.}
    \end{columns}
\end{frame}
