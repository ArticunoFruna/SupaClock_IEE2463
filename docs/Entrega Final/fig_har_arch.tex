\begin{tikzpicture}[
    ...
    dims/.style={font=\fontsize{5}{6}\selectfont\itshape, color=black!55, inner sep=0.5pt},
    flow/.style={->, >=stealth, thick, black!70},
    group/.style={draw=black!35, dashed, thick, rounded corners=6pt, inner sep=8pt}
]

% Fila de nodos horizontal (más separados entre bloques conceptuales para evitar cruce de líneas punteadas)
\node[stagebox] (in)   {Entrada\\IMU 6ch};
\node[cnnbox, right=0.55cm of in]   (c1) {Conv1D\\32 $\times$ 5};
\node[cnnbox, right=0.35cm of c1]   (p1) {MaxPool\\/2};
\node[cnnbox, right=0.75cm of p1]   (c2) {Conv1D\\64 $\times$ 5};
\node[cnnbox, right=0.35cm of c2]   (p2) {MaxPool\\/2};
\node[cnnbox, right=0.75cm of p2]   (c3) {Conv1D\\128 $\times$ 3};
\node[cnnbox, right=0.35cm of c3]   (gap) {GAP};
\node[cnnbox, right=0.75cm of gap]  (fc)  {Dense\\64};
...
% Anotaciones de forma tensorial (subidas un poco a below=0.01cm para que no se crucen con la línea punteada)
\node[dims, below=0.01cm of in]   {200$\times$6};
\node[dims, below=0.01cm of c1]   {200$\times$32};
\node[dims, below=0.01cm of p1]   {100$\times$32};
\node[dims, below=0.01cm of c2]   {100$\times$64};
\node[dims, below=0.01cm of p2]   {50$\times$64};
\node[dims, below=0.01cm of c3]   {50$\times$128};
\node[dims, below=0.01cm of gap]  {128};
\node[dims, below=0.01cm of fc]   {64};
\node[dims, below=0.01cm of out]  {4};
