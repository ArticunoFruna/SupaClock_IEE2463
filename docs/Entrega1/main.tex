...
Sin embargo, a diferencia del mercado de consumo masivo, el diseño de este dispositivo no se plantea bajo el paradigma de un ``reloj inteligente comercial'' genérico. Investigaciones en estudios psicológicos y de interacción humano-computadora han demostrado que la hiperconectividad y la avalancha de notificaciones en teléfonos móviles interactivos y \textit{smartwatches} incrementan severamente la carga cognitiva, induciendo síntomas estrechamente relacionados con la hiperactividad y el déficit de atención clínico \citep{Kushlev2016, Pielot2014}. Es por esto que la principal postura arquitectónica de este wearable se funda en el minimalismo y el principio de \textbf{``Cero Distracciones''}. El dispositivo actuará como una interfaz biométrica estrictamente diseñada para recolectar datos inerciales y cardíacos en segundo plano, promoviendo objetivamente un impacto en la salud \citep{Shin2019} sin bombardear al paciente con vibraciones de redes sociales. Así, el ecosistema de \textit{SupaClock} relega el procesamiento y consumo energético exclusivamente al hardware dedicado a telemetría médica.
...
\subsubsection{Contador de pasos algorítmico}

Dadas las restricciones del proyecto de no utilizar módulos comerciales pre-programados, se desarrollará un algoritmo propietario en C. La \rtabla{tab:step_algos} presenta una comparación de las principales familias de algoritmos de conteo de pasos reportados en la literatura para dispositivos wearable.

\begin{table}[H]
    \centering
    \renewcommand{\arraystretch}{1.3}
    \begin{tabular}{|L{3cm}|c|c|L{4cm}|L{4cm}|}
        \hline
        \rowcolor[HTML]{E5E7E9}
        \textbf{Algoritmo}                        & \textbf{Precisión} & \textbf{Costo comp.} & \textbf{Ventajas}                                                  & \textbf{Desventajas}                                                           \\ \hline
        \textbf{Detección de picos con umbral adaptativo (Seleccionado)} & 92--96\%           & Muy bajo             & Simple, bajo consumo de RAM, robusto ante cambios de velocidad     & Sensible a vibraciones; requiere filtrado previo                               \\ \hline
        Cruce por cero (\textit{Zero-Crossing})   & 85--90\%           & Muy bajo             & Implementación trivial, sin parámetros de umbral                   & Alta tasa de falsos positivos con movimientos no periódicos                    \\ \hline
        Autocorrelación temporal                  & 93--97\%           & Alto                 & Excelente con señales periódicas, estima frecuencia de paso        & Costoso en CPU y RAM (requiere buffers grandes); no apto para tiempo real en C3 \\ \hline
        Clasificación por ML (CNN/LSTM)           & 95--98\%           & Muy alto             & Mayor precisión, distingue tipos de paso                           & Excesivo para un simple contador; ya se usa ML para clasificación de actividad  \\ \hline
    \end{tabular}
    \caption{Comparación de algoritmos de conteo de pasos para dispositivos wearable.}
    \label{tab:step_algos}
\end{table}

Se seleccionó el algoritmo de \textbf{detección de picos con umbral adaptativo} por su excelente balance entre precisión (92--96\%) y costo computacional mínimo, siendo el más adecuado para la arquitectura RISC-V del ESP32-C3 \citep{esp32c3_ds} que carece de FPU por hardware. El pipeline de procesamiento opera sobre la señal del acelerómetro BMI160 \citep{bmi160_ds} y consta de las siguientes etapas:

\begin{enumerate}
    \item \textbf{Cálculo de magnitud:} Se computa $|a| = \sqrt{a_x^2 + a_y^2 + a_z^2}$ a partir de las lecturas del acelerómetro a 50~Hz, eliminando la dependencia de la orientación del dispositivo en la muñeca.
    \item \textbf{Filtro pasabajos (Media Móvil):} Se aplica un filtro de media móvil de ventana $N = 10$ muestras (200~ms a 50~Hz), con el propósito de atenuar vibraciones de alta frecuencia y ruido mecánico. La frecuencia de corte resultante es $f_c \approx \frac{f_s}{2\pi N} \approx 0.8$~Hz, que retiene las componentes de paso humano típicas (1--3~Hz caminando, hasta 4~Hz corriendo).
    \item \textbf{Detección de picos con umbral adaptativo:} Se implementa un detector de cruce por umbral con histéresis. El umbral se recalcula cada 5~segundos como el 60\% de la amplitud pico-a-pico observada en la ventana anterior. Un paso se contabiliza cuando la señal filtrada cruza el umbral ascendente seguido de un cruce descendente, con un período refractario mínimo de 250~ms entre detecciones consecutivas para evitar doble conteo.
    \item \textbf{Validación temporal:} Se requiere un mínimo de 3 pasos consecutivos dentro de un intervalo de 5~segundos para confirmar que el usuario está caminando (evitando falsos positivos por movimientos aislados).
\end{enumerate}

Este contador contará con la funcionalidad de ser reseteado a cero a voluntad del usuario navegando por los menús de la interfaz gráfica local del \textit{smartwatch}.

\subsubsection{Clasificación de actividades (4 estados)}

Se requiere implementar un modelo de clasificación capaz de predecir cuatro estados físicos y situaciones de riesgo: \textit{reposo}, \textit{caminar}, \textit{correr} y detección de \textit{caída/emergencia}. Este último estado es crítico para otorgar un valor añadido a la seguridad del usuario.

La \rtabla{tab:ml_models} presenta un análisis comparativo de arquitecturas de modelos de \textit{Machine Learning} candidatos, basándose en benchmarks de HAR (\textit{Human Activity Recognition}) en dispositivos edge \citep{har_tinyml}.

\begin{table}[H]
    \centering
    \renewcommand{\arraystretch}{1.3}
    \begin{tabular}{|L{3cm}|c|c|c|L{4.5cm}|}
        \hline
        \rowcolor[HTML]{E5E7E9}
        \textbf{Modelo}                   & \textbf{Peso (KB)} & \textbf{Latencia (ms)} & \textbf{Precisión est.} & \textbf{Observaciones}                                                                        \\ \hline
        Decision Tree / Random Forest      & 2--10             & $<$1                    & 85--90\%                 & Bajo consumo de RAM. Interpretable. Limitado con patrones temporales complejos.             \\ \hline
        SVM (RBF Kernel)                   & 5--20              & 1--5                    & 88--92\%                 & Buen rendimiento con features manuales. Requiere extracción de características previa.        \\ \hline
        \textbf{CNN 1D (Seleccionado)}     & 15--50             & 5--15                   & 92--96\%                 & Opera directamente sobre ventanas de aceleración cruda. Compatible con TFLite Micro y Edge Impulse. \\ \hline
        LSTM / GRU                         & 50--200            & 20--50                  & 93--97\%                 & Excelente para series temporales, pero excesivo en memoria y latencia para ESP32-C3.          \\ \hline
    \end{tabular}
    \caption{Comparación de arquitecturas de ML para clasificación de actividad en microcontroladores \citep{har_tinyml}.}
    \label{tab:ml_models}
\end{table}

Se ha seleccionado una \textbf{Red Neuronal Convolucional 1D (CNN 1D)} como arquitectura base, compuesta por 2 capas convolucionales (8 y 16 filtros respectivamente, kernel de tamaño 3) seguidas de una capa de \textit{Global Average Pooling} y una capa densa de 4 neuronas con activación \textit{softmax}. Esta arquitectura ha demostrado un rendimiento superior en tareas de HAR sobre microcontroladores frente a modelos clásicos \citep{har_tinyml}, y se entrenará utilizando \textbf{Edge Impulse} \citep{edge_impulse} con datos recolectados del propio BMI160 \citep{bmi160_ds} montado en la muñeca, para luego exportar el modelo cuantizado a \texttt{int8} mediante \textbf{TensorFlow Lite for Microcontrollers} \citep{tflite_micro}. Los objetivos de diseño del modelo son:

\begin{itemize}
    \item \textbf{Peso del modelo:} $\leq$ 30~KB (flash) y $\leq$ 10~KB (arena de inferencia en SRAM).
    \item \textbf{Latencia de inferencia:} $\leq$ 20~ms en ESP32-C3 a 160~MHz \citep{esp32c3_ds}.
    \item \textbf{Precisión objetivo:} $\geq$ 90\% en las 4 clases, con énfasis en minimizar falsos negativos para la detección de caídas.
\end{itemize}

\subsubsection{Justificación de la arquitectura Edge AI}
Se ha decidido implementar el modelo de Inteligencia Artificial directamente en el microcontrolador (\textit{Edge Computing/TinyML}) en lugar de procesarlo en la nube. Esta decisión se justifica por tres factores clave: minimiza la latencia (aspecto crítico para reportar una caída en tiempo real), reduce drásticamente el consumo energético al evitar la transmisión constante de telemetría cruda por la antena Bluetooth, y garantiza la privacidad de los movimientos del usuario.

\subsection{Especificación de manejo de datos}

Se detallan las especificaciones de los datos que el sistema genera, almacena y transmite. La \rtabla{tab:datos} presenta el volumen, formato y destino de cada tipo de dato.

\begin{table}[H]
    \centering
    \renewcommand{\arraystretch}{1.3}
    \begin{tabular}{|L{3cm}|c|c|L{3cm}|L{3.5cm}|}
        \hline
        \rowcolor[HTML]{E5E7E9}
        \textbf{Tipo de dato}    & \textbf{Tamaño} & \textbf{Frecuencia}  & \textbf{Formato}                          & \textbf{Destino}                               \\ \hline
        Telemetría continua (HR, SpO2, Temp, Pasos, Actividad) & 12 bytes        & 1 Hz (BLE Notify) & Struct empaquetado Little-Endian (uint8, int16, uint32) & Buffer local $\rightarrow$ BLE $\rightarrow$ Firestore \\ \hline
        ECG crudo (AD8232)       & 2 bytes/muestra & 100 Hz (30 s)      & int16 (12-bit ADC en 16-bit container) \citep{ad8232_ds}    & BLE streaming $\rightarrow$ Cloud Functions    \\ \hline
        Ventana IMU (ML)         & 600 bytes       & Cada 2 s           & 6 $\times$ int16 $\times$ 50 muestras \citep{bmi160_ds}     & Procesado local (TinyML)                       \\ \hline
        Sesión (resumen)         & $\sim$200 bytes & Cada 5 min         & JSON (Firestore document)                 & Cloud Firestore (persistencia) \citep{firebase_docs}                 \\ \hline
        ECG procesado            & $\sim$500 bytes & Bajo demanda       & JSON (BPM, HRV, R-peaks)                  & Cloud Functions $\rightarrow$ Firestore        \\ \hline
    \end{tabular}
    \caption{Especificación de volumen, formato y destino de los datos del sistema.}
    \label{tab:datos}
\end{table}

La velocidad de transmisión BLE se configura a un \textit{connection interval} de 15~ms con un MTU de 247~bytes \citep{ble_core_spec}, lo que permite un throughput efectivo de aproximadamente 13~kB/s. Este ancho de banda es suficiente para transmitir la telemetría continua (12~B/s) y los datos de ECG crudo ($200$~B/s a 100~Hz). El almacenamiento local en el microcontrolador utiliza un buffer circular de 4~KB en SRAM \citep{esp32c3_ds} para la telemetría, sin requerir almacenamiento persistente en flash, ya que los datos se transmiten en tiempo real.
...
Respecto a la fuente de poder, se seleccionó una \textbf{batería de polímero de litio (LiPo) de 3.7V}. Para justificar la capacidad seleccionada, la \rtabla{tab:consumo} presenta un desglose detallado del consumo energético estimado de cada subsistema, basándose en los valores típicos de las hojas de datos de cada componente.
...
\begin{table}[H]
    ...
    \begin{tabular}{|L{4cm}|c|c|c|}
        \hline
        \rowcolor[HTML]{E5E7E9}
        \textbf{Componente}          & \textbf{Modo Activo (mA)} & \textbf{Modo Reposo (mA)} & \textbf{Ciclo de uso (\%)} \\ \hline
        ESP32-C3 (CPU + Radio off) \citep{esp32c3_ds}   & 15.0                       & 0.05                       & 100                         \\ \hline
        ESP32-C3 (BLE Tx, periódico) \citep{esp32c3_ds} & 5.0                        & 0.0                        & 10                          \\ \hline
        Pantalla ST7789 (backlight) \citep{st7789_ds}  & 25.0                       & 0.0                        & 40                          \\ \hline
        MAX30102 (PPG activo) \citep{max30102_ds}        & 0.6                        & 0.0                        & 50                          \\ \hline
        MAX30205 (Temperatura) \citep{max30205_ds}       & 0.6                        & 0.01                       & 5                           \\ \hline
        BMI160 (IMU normal) \citep{bmi160_ds}          & 0.9                        & 0.003                      & 100                         \\ \hline
        MAX17048 (Fuel Gauge) \citep{max17048_ds}        & 0.05                       & 0.05                       & 100                         \\ \hline
        AD8232 (ECG, bajo demanda) \citep{ad8232_ds}   & 0.17                       & 0.0                        & 2                           \\ \hline
        LDO ME6211 ($I_Q$) \citep{me6211_ds}           & 0.055                      & 0.055                      & 100                         \\ \hline
        \rowcolor[HTML]{D5F5E3}
        \textbf{Total ponderado}     & \textbf{$\sim$22.9}        & \textbf{$\sim$0.17}        & ---                         \\ \hline
    \end{tabular}
    \caption{Desglose del consumo energético estimado por componente (valores de hojas de datos).}
    \label{tab:consumo}
\end{table}

Considerando un perfil de uso diario de \textbf{8 horas de modo activo} (pantalla encendida, sensores operando) y \textbf{16 horas de modo reposo} (pantalla apagada, solo IMU activo), el consumo promedio ponderado resulta:

\begin{equation}
    I_{avg} = \frac{8 \times 22.9 + 16 \times 0.17}{24} \approx 7.7~\text{mA}
\end{equation}

Con una batería de \textbf{250~mAh} (modelo 502030, dimensiones 30$\times$20$\times$5~mm, con circuito de protección PCM integrado) y considerando una eficiencia del LDO del 90\% \citep{me6211_ds}, la autonomía teórica es:

\begin{equation}
    t_{autonomia} = \frac{250 \times 0.9}{7.7} \approx 29.2~\text{horas} \approx 1.2~\text{días}
\end{equation}

Este cálculo valida la selección de la celda de 250~mAh, superando holgadamente el requerimiento mínimo de 12~horas de operación.

\textbf{Consideraciones mecánicas:} El diseño contempla una arquitectura tipo \textit{sandwich} de doble PCB (PCB superior con MCU y conector de pantalla, batería en el centro, PCB inferior con los sensores de contacto con la piel: MAX30102 y MAX30205). Se utilizará la misma celda 502030 en ambas versiones del producto. La \rtabla{tab:stackup} presenta el análisis de apilamiento (\textit{stack-up}).

\begin{table}[H]
    \centering
    \renewcommand{\arraystretch}{1.3}
    \begin{tabular}{|L{4.5cm}|c|c|}
        \hline
        \rowcolor[HTML]{E5E7E9}
        \textbf{Capa}                              & \textbf{Prototipo (mm)} & \textbf{Versión Pro (mm)} \\ \hline
        Display + cristal                           & 2.5                      & 2.0                  \\ \hline
        Top PCB (MCU, conectores)                   & 1.6                      & 1.0                  \\ \hline
        Batería LiPo 502030                         & 5.0 (250 mAh)            & 5.0 (250 mAh)        \\ \hline
        Bottom PCB (sensores skin-contact)          & 1.6                      & 1.0                  \\ \hline
        Cover posterior (ventana óptica PPG)        & 0.5                      & 0.5                  \\ \hline
        Carcasa (paredes superior e inferior)       & 1.5                      & 1.2                  \\ \hline
        \rowcolor[HTML]{D5F5E3}
        \textbf{Espesor total}                     & \textbf{$\sim$12.7}      & \textbf{$\sim$10.7}   \\ \hline
        \rowcolor[HTML]{FFF9C4}
        \textit{Referencia: Apple Watch (10.7), Galaxy Watch (9.8)} & \multicolumn{2}{c|}{\textit{Rango comercial: 9--11 mm}} \\ \hline
    \end{tabular}
    \caption{Análisis de apilamiento mecánico (\textit{stack-up}) para la arquitectura sandwich de doble PCB.}
    \label{tab:stackup}
\end{table}

La huella de 20$\times$30~mm de la celda 502030 se ajusta al espacio disponible bajo la pantalla de 1.69'' ($\sim$35$\times$42~mm), dejando margen para el ruteo de cables y conectores FPC. La versión ``Pro'' (PCB SMD de 4 capas fabricada en JLCPCB) reduce el espesor total a $\sim$10.7~mm gracias a PCBs de 1.0~mm y una carcasa más delgada, igualando el perfil de un Apple Watch.

Para la gestión térmica y energética de dicha celda, la \rtabla{tab:bms_comparison} resume el análisis del subsistema de carga (BMS). La combinación de los integrados genéricos TP4056 y el circuito de protección DW01A es ideal para el primer prototipo. Este módulo se adquiere fácilmente soldado en placa base con puerto Type-C integrado por un valor bajo, superando las barreras de soldadura que ofrecen QFNs como el BQ24075.
...
\begin{table}[H]
    ...
    \begin{tabular}{|l|c|c|r|}
        ...
        Batería LiPo 3.7V 250 mAh (502030) & 1            & Alimentación         & \$7.980              \\ \hline
        ...
        \rowcolor[HTML]{D5F5E3}
        \textbf{Subtotal (Prototipo)} & ---               & ---                  & \textbf{\$58.301}    \\ \hline
        PCB SMD (JLCPCB/PCBWay)       & 1                 & Versión Pro (Bonus)  & \$30.000             \\ \hline
        \rowcolor[HTML]{ABEBC6}
        \textbf{Total con Bonus}      & ---               & ---                  & \textbf{\$88.301}    \\ \hline
    \end{tabular}
...
% ==========================================
% AVANCE 1 (5%) — SECCIONES NUEVAS
% ==========================================

\newpage
\part*{Informe de Avance 1 (5\%)}
\addcontentsline{toc}{part}{Informe de Avance 1 (5\%)}

% ==========================================
% A1. DIAGRAMA DE BLOQUES ACTUALIZADO
% ==========================================
\section{Diagramas de bloques de bajo nivel actualizados}

El diagrama de bloques de alto nivel del sistema SupaClock (presentado en la \rfigura{fig:bloques}) se mantiene \textbf{sin modificaciones} respecto a la entrega de planificación. Sin embargo, tras validar la integración de los componentes y el ecosistema de desarrollo, se han diseñado diagramas de bajo nivel para detallar el funcionamiento interno de la concurrencia del firmware y el flujo de los datos biomédicos hacia la nube.

\subsection{Arquitectura de Firmware (FreeRTOS)}
El microcontrolador gestiona la concurrencia a través del RTOS. Para el entorno principal (\texttt{env:main}), se implementaron tareas independientes para la lectura de sensores (específicamente temperatura en esta iteración) y la actualización de la interfaz gráfica local. Las tareas están separadas lógicamente y se comunican mediante colas de mensajes estructuradas (\texttt{xQueue}), garantizando la concurrencia segura. La tarea encargada de las notificaciones BLE (\texttt{TaskBLE}) se encuentra delineada en la arquitectura, pero no se ha activado ya que la aplicación móvil no es parte del alcance técnico de esta entrega.
...
\begin{figure}[H]
    ...
    \caption{Arquitectura de tareas concurrentes (FreeRTOS) implementada en el ESP32-C3, destacando el flujo de lectura de sensores y visualización gráfica local.}
    \label{fig:freertos}
\end{figure}
\shorthandon{>}\shorthandon{<}

\subsection{Pipeline de Datos Biomédicos}
El flujo de datos desde la captura en hardware hasta el procesamiento analítico en la nube se modela en una tubería (\textit{pipeline}). A pesar de que los datos biomédicos complejos del ECG y la fotopletismografía aún no entran en funcionamiento, la arquitectura subyacente para su ingesta, digitalización, transmisión BLE y almacenamiento en Firebase está completamente definida en el backend implementado.
...
\begin{figure}[H]
    ...
    \caption{Pipeline de adquisición y procesamiento de datos biomédicos, modelando hacia el futuro el recorrido completo de la señal.}
    \label{fig:pipeline}
\end{figure}
\shorthandon{>}\shorthandon{<}

% ==========================================
% A2. PLANIFICACIÓN ACTUALIZADA
% ==========================================
\section{Planificación actualizada}

\subsection{Estado de avance respecto a la planificación}

La \rtabla{tab:avance} presenta el contraste entre las tareas planificadas para el hito del 5\% y el estado actual de ejecución.

\begin{table}[H]
    \centering
    \renewcommand{\arraystretch}{1.3}
    \begin{tabular}{|L{5cm}|c|L{6cm}|}
        \hline
        \rowcolor[HTML]{E5E7E9}
        \textbf{Tarea planificada (5\%)}     & \textbf{Estado}        & \textbf{Observaciones}                                                                      \\ \hline
        Prueba preliminar de componentes     & \textcolor{green!60!black}{\textbf{Completado}} & Se probaron: ST7789, MAX30205, MAX30102 y BMI160 de forma individual.                       \\ \hline
        Crear base de datos en Firebase      & \textcolor{green!60!black}{\textbf{Completado}} & Cloud Firestore desplegado con estructura de colecciones y reglas de seguridad.              \\ \hline
        Crear servidor en Firebase           & \textcolor{green!60!black}{\textbf{Completado}} & Cloud Functions desplegadas con el algoritmo Pan-Tompkins \citep{pan_tompkins} en Python.                        \\ \hline
        Crear repositorio en GitHub          & \textcolor{green!60!black}{\textbf{Completado}} & Repositorio activo con estructura modular PlatformIO y múltiples entornos de test.          \\ \hline
        \rowcolor[HTML]{FFF9C4}
        Driver de pantalla LCD (adelantado)  & \textcolor{green!60!black}{\textbf{Completado}} & Driver ST7789 funcional con SPI a 80~MHz y soporte DMA. Integración con LVGL 8.4 \citep{lvgl_ref}.          \\ \hline
        \rowcolor[HTML]{FFF9C4}
        Decodificación JPEG en MCU (adelantado)  & \textcolor{green!60!black}{\textbf{Completado}} & Algoritmo JPEGDEC \citep{jpegdec_ref} integrado con SPIFFS \citep{espspiffs_ref} para renderizar imágenes directamente en pantalla.   \\ \hline
        \rowcolor[HTML]{FFF9C4}
        Prueba de BLE con NimBLE (adelantado)    & \textcolor{green!60!black}{\textbf{Completado}} & Stack BLE funcional con servicios GATT, bonding y notificaciones de telemetría simulada \citep{nimble_ref}.    \\ \hline
    \end{tabular}
    \caption{Contraste entre avance esperado (5\%) y avance logrado. Las filas amarillas indican tareas adelantadas respecto a la planificación original.}
    \label{tab:avance}
\end{table}

Como se observa, todas las tareas planificadas para el hito del 5\% han sido completadas satisfactoriamente. Adicionalmente, se realizaron avances no previstos originalmente para este hito (resaltados en amarillo), como la integración completa del driver de pantalla con LVGL, la decodificación de imágenes JPEG en el microcontrolador y la implementación del stack BLE con NimBLE. Estos avances adelantan trabajo correspondiente al hito del 25\%.

\subsection{Actualización de la Carta Gantt}

La carta Gantt presentada en la \rfigura{fig:gantt} se mantiene como referencia base. Respecto a ella, se registran los siguientes ajustes producto del avance real:

\begin{itemize}
    \item \textbf{Tareas adelantadas:} Las actividades de ``Driver de pantalla'', ``Integración LVGL'' y ``Prueba BLE'' originalmente planificadas para el hito del 25\% fueron completadas durante el hito del 5\%, ya que resultaron ser prerrequisitos naturales para validar la arquitectura de firmware.
    \item \textbf{Tareas repriorizadas:} La ``Integración inicial de sensores en conjunto'' (planificada para 25\%) se mantiene en su fecha original, ya que requiere la implementación previa del subsistema de energía (BMS + LDO) para operar los sensores de forma autónoma sin alimentación USB.
    \item \textbf{Sin atrasos:} No se reportan tareas atrasadas respecto a la planificación original.
\end{itemize}

\subsection{Distribución del trabajo}

Para esta primera etapa, el equipo adoptó un enfoque de \textbf{trabajo colaborativo integral}, donde los tres integrantes participaron activamente en todas las áreas del proyecto. La distribución de roles fue la siguiente:

\begin{itemize}
    \item \textbf{Tomás Avendaño:} Participó en las pruebas de sensores (MAX30205, MAX30102), implementación del stack BLE con NimBLE y despliegue del backend Firebase.
    \item \textbf{Benjamín Sepúlveda:} Contribuyó en el cableado de prototipos, verificación de niveles de voltaje en protoboard, pruebas del bus I2C y documentación del informe.
    \item \textbf{Pablo Uribe:} Participó en la implementación del driver ST7789, integración de JPEGDEC y LVGL, y actuó como \textbf{verificador del trabajo conjunto}, asegurando la coherencia entre los módulos desarrollados por cada integrante.
\end{itemize}

% ==========================================
% A3. IMPLEMENTACIÓN Y RESULTADOS
% ==========================================
\section{Implementación y resultados}

En esta sección se describen los bloques implementados durante este primer hito de avance. Cada subsección detalla la funcionalidad del bloque, las pruebas realizadas y los resultados obtenidos.

\subsection{Bloque de Interfaz Local: Driver de pantalla ST7789}

\subsubsection{Descripción y funcionalidad}
Se implementó un driver completo en C para el controlador de pantalla ST7789 \citep{st7789_ds} (pantalla IPS LCD de 1.69'', 240$\times$280 píxeles, RGB565). El driver opera sobre el bus SPI del ESP32-C3 \citep{esp32c3_ds} a una velocidad de reloj de \textbf{80~MHz}, utilizando DMA automático (\texttt{SPI\_DMA\_CH\_AUTO}) para transferencias asíncronas que liberan al procesador durante el volcado de píxeles.

Las funciones principales implementadas son:
\begin{itemize}
    \item \texttt{st7789\_init()}: Configura el bus SPI, ejecuta la secuencia de inicialización del controlador (software reset, sleep out, inversión de color para IPS, modo RGB565 de 16 bits) y enciende la pantalla.
    \item \texttt{st7789\_fill\_screen(color)}: Pinta la pantalla completa de un color uniforme, enviando los datos en bloques de 40 líneas.
    \item \texttt{st7789\_draw\_bitmap(x, y, w, h, bitmap)}: Renderiza un bloque rectangular de píxeles en la posición especificada, con recorte automático en los bordes para evitar artefactos (\textit{ghosting}).
\end{itemize}

Un detalle técnico relevante es la compensación de \textit{offset} vertical de 20 píxeles en la función \texttt{set\_addr\_window()}, necesaria porque la RAM interna del ST7789 tiene una resolución de 320 líneas \citep{st7789_ds}, pero la pantalla física solo expone 280.

\subsubsection{Integración con LVGL 8.4}
Se integró la librería LVGL versión 8.4 \citep{lvgl_ref}. Se implementó un callback \texttt{flush\_cb} que conecta el lienzo virtual de LVGL con \texttt{st7789\_draw\_bitmap()}, permitiendo renderizar widgets (etiquetas, barras de progreso, animaciones) de forma eficiente.

El buffer de dibujo parcial se configuró en \textbf{14.4~KB} (240~$\times$~30 líneas $\times$ 2~bytes/píxel), permitiendo un uso eficiente de la SRAM del ESP32-C3 (400~KB) \citep{esp32c3_ds}.

\subsubsection{Resultados}
Se verificó el correcto funcionamiento mostrando textos con anti-aliasing, barras de progreso animadas y colores vibrantes a $\sim$30~FPS.

\subsection{Bloque de Interfaz Local: Decodificación JPEG (JPEGDEC)}

\subsubsection{Descripción y funcionalidad}
Se integró la librería \textbf{JPEGDEC} \citep{jpegdec_ref} para decodificar imágenes JPEG almacenadas en la partición SPIFFS \citep{espspiffs_ref} del ESP32 directamente hacia la pantalla, sin almacenar el bitmap descomprimido en RAM. JPEGDEC decodifica en bloques MCU de 16$\times$16 píxeles, invocando un callback (\texttt{JPEGDraw}) que envía cada bloque al ST7789 vía \texttt{st7789\_draw\_bitmap()}.

Este enfoque de \textit{streaming} es fundamental: una imagen de 240$\times$280 en RGB565 ocuparía 134.4~KB descomprimida (más del 30\% de la SRAM), mientras que el JPEG comprimido ocupa 10--20~KB en flash con un overhead de RAM de solo $\sim$4~KB.

\subsubsection{Resultados}
Se logró decodificar y mostrar imágenes JPEG de 240$\times$280 almacenadas en SPIFFS. El tiempo de renderizado completo es de 200--400~ms.

\subsection{Bloque de Sensores: Prueba del MAX30102 (PPG)}

\subsubsection{Descripción y funcionalidad}
Se implementó un driver inicial para el sensor fotopletismógrafo \textbf{MAX30102} \citep{max30102_ds}, operando en el bus I2C a la dirección \texttt{0x57}. El driver implementa inicialización (software reset, modo SpO2, ajuste de LEDs) y lectura de la FIFO (muestras de 18 bits para canales rojo e IR).

La lectura de datos accede al registro \texttt{FIFO\_DATA (0x07)} \citep{max30102_ds}, donde cada muestra consiste en 6 bytes (3 por canal), enmascarados a 18 bits significativos.

\subsubsection{Resultados}
Se verificó la comunicación I2C confirmando la lectura correcta del registro \texttt{PART\_ID (0xFF) = 0x15} \citep{max30102_ds}. Se obtuvieron lecturas crudas de señales roja e IR con variación al colocar el dedo sobre el sensor. La calibración algorítmica para SpO2 y BPM será implementada en la siguiente etapa.

\subsection{Bloque de Comunicación: BLE con NimBLE}

\subsubsection{Descripción y funcionalidad}
Se implementó el stack Bluetooth Low Energy utilizando la librería \textbf{Apache NimBLE} \citep{nimble_ref} integrada en ESP-IDF. Se configuraron dos servicios GATT:

\begin{itemize}
    \item \textbf{Device Information Service (0x180A):} Expone el nombre del fabricante (``ArticunoFruna'') y el modelo (``SupaClock-C3'') como características de solo lectura.
    \item \textbf{SupaClock Telemetry Service (UUID personalizado):} Contiene tres características con notificaciones: HR/SpO2, Temperatura y Pasos.
\end{itemize}

Se configuró el \textit{Appearance} como ``Wristwatch'' (0x00C1) según la especificación BLE \citep{ble_core_spec}, y se habilitó \textit{Secure Connections} (SC) con bonding para persistir el emparejamiento entre sesiones. La publicidad BLE se dividió en dos paquetes: el principal con flags y UUID del servicio, y el \textit{Scan Response} con el nombre del dispositivo.

\subsubsection{Resultados}
El dispositivo es visible y emparejable desde cualquier teléfono Android/iOS. Se verificó la recepción de notificaciones con datos de telemetría simulada (dummies) a 1~Hz.

\subsection{Bloque de Backend: Firebase y Cloud Firestore}

\subsubsection{Descripción y funcionalidad}
Se desplegó el ecosistema de backend utilizando \textbf{Firebase} \citep{firebase_docs}:

\textbf{Cloud Firestore} se configuró con la siguiente jerarquía:
\begin{itemize}
    \item \texttt{users/\{userId\}}: Perfil del usuario.
    \item \texttt{users/.../sessions/\{sessionId\}}: Sesiones con métricas agregadas.
    \item \texttt{users/.../ecgReadings/\{readingId\}}: Lecturas de ECG crudas y procesadas.
\end{itemize}

\textbf{Cloud Functions} (Python 3.12) implementa:
\begin{itemize}
    \item \texttt{process\_ecg}: Trigger automático que ejecuta el pipeline \textbf{Pan-Tompkins} \citep{pan_tompkins} completo (filtrado pasabanda, derivación, cuadratura, integración de ventana móvil, detección de picos R con umbral adaptativo). Calcula BPM y HRV (SDNN).
    \item \texttt{compute\_daily\_summary}: Función callable que agrega métricas diarias.
\end{itemize}

\subsubsection{Resultados}
Se verificó el correcto despliegue mediante pruebas con datos sintéticos. El algoritmo Pan-Tompkins detecta correctamente picos R y calcula BPM dentro de rangos fisiológicamente plausibles (60--100 BPM para adulto en reposo simulado).

\subsection{Evaluación general y próximos pasos}

Los resultados de este primer hito validan la viabilidad de la arquitectura. Todos los componentes probados individualmente operan correctamente a través de las interfaces planificadas (I2C y SPI). El backend está operativo y listo para recibir datos reales.

Para el siguiente hito (25\%), los esfuerzos se concentrarán en:
\begin{itemize}
    \item Integración simultánea de múltiples sensores en el bus I2C (con mutex FreeRTOS \citep{freertos_ref}).
    \item Implementación del subsistema de energía (BMS TP4056 \citep{tp4056_ds} + LDO + Fuel Gauge).
    \item Desarrollo del algoritmo de conteo de pasos sobre datos reales del BMI160 \citep{bmi160_ds}.
    \item Inicio de la recolección de datos de entrenamiento para el modelo de ML.
\end{itemize}
...
\subsection{Anexo A: Código del driver ST7789 (extracto principal)}

\begin{lstlisting}[caption={Función de inicialización del driver ST7789.}, label={lst:st7789_init}]
esp_err_t st7789_init(void) {
    // GPIO Config para pines DC y RST
    gpio_config_t gc = {
        .pin_bit_mask = (1ULL << ST7789_DC_PIN)
                      | (1ULL << ST7789_RST_PIN),
        .mode = GPIO_MODE_OUTPUT,
    };
    gpio_config(&gc);

    // SPI Bus a 80 MHz con DMA automatico
    spi_bus_config_t buscfg = {
        .mosi_io_num = ST7789_MOSI_PIN,
        .sclk_io_num = ST7789_SCK_PIN,
        .max_transfer_sz = 240 * 280 * 2 + 8
    };
    spi_bus_initialize(SPI2_HOST, &buscfg,
                       SPI_DMA_CH_AUTO);

    // Hardware Reset + Init Sequence
    lcd_cmd(spi, ST7789_SWRESET);
    lcd_cmd(spi, ST7789_SLPOUT);
    lcd_cmd(spi, ST7789_INVON); // IPS requiere inversion
    lcd_cmd(spi, ST7789_DISPON);
    return ESP_OK;
}
\end{lstlisting}

\subsection{Anexo B: Callback de decodificación JPEG}

\begin{lstlisting}[caption={Callback de renderizado JPEG por bloques MCU.}, label={lst:jpegdraw}, language=C++]
// Callback que envia bloques 16x16 decodificados
// directamente al ST7789 sin almacenar en RAM
int JPEGDraw(JPEGDRAW *pDraw) {
    ...
    return 1; // Continuar decodificacion
}

// Uso: decodificar desde SPIFFS
jpeg.open("/spiffs/foto.jpg", myOpen, myClose,
          myRead, mySeek, JPEGDraw);
jpeg.setPixelType(RGB565_BIG_ENDIAN);
jpeg.decode(0, 0, 0);
\end{lstlisting}

\subsection{Anexo C: Driver MAX30102 (lectura FIFO)}

\begin{lstlisting}[caption={Lectura de muestras desde la FIFO del MAX30102.}, label={lst:max30102}]
esp_err_t max30102_read_fifo(uint32_t *red,
                              uint32_t *ir) {
    uint8_t reg = MAX30102_REG_FIFO_DATA;
    uint8_t data[6];
    esp_err_t err = i2c_master_write_read_device(
        I2C_MASTER_NUM, MAX30102_I2C_ADDR,
        &reg, 1, data, 6,
...
\subsection{Anexo D: Extracto de Cloud Functions (Pan-Tompkins)}

\begin{lstlisting}[caption={Trigger de procesamiento de ECG en Cloud Functions.}, label={lst:ecg_cloud}, language=Python]
@firestore_fn.on_document_created(
    document="users/{userId}/sessions/"
             "{sessionId}/ecgReadings/{readingId}",
    region="us-central1",
    memory=options.MemoryOption.MB_256,
)
def process_ecg(event):
    data = event.data.to_dict()
    raw_data = data.get("rawData", [])
    fs = data.get("samplingRate", 100.0)

    # Pipeline Pan-Tompkins completo
    results = pan_tompkins(raw_data, fs)

    # Actualizar documento con resultados
    event.data.reference.update({
        "processedBPM": results["bpm"],
        "hrv": results["hrv"],
        "rPeaks": results["r_peaks"],
